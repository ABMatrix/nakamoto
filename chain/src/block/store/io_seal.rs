...
fn get<H: Decodable, S: Seek + Read>(mut stream: S, ix: u64, sk: Vec<u8>) -> Result<H, Error> {
    let size = std::mem::size_of::<H>() + seal::BLOCK_LEN;
...
impl<H: Decodable> FileReader<H> {
    ...
    fn new(file: fs::File, seal: Vec<u8>) -> Self {
        Self {
            file,
            queue: VecDeque::new(),
            index: 0,
...
impl<H: 'static + Copy + Encodable + Decodable> Store for File<H> {
    ...
    fn put<I: Iterator<Item = Self::Header>>(&mut self, headers: I) -> Result<Height, Error> {
        self::put(&mut self.file, headers, self.seal.clone())
    }
    ...
    fn get(&self, height: Height) -> Result<H, Error> {
        if let Some(ix) = height.checked_sub(1) {
            ...
            get(&mut file, ix, self.seal.clone())
        } else {
            Ok(self.genesis)
        }
    }
